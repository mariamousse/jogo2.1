...
function draw() {
 ...
 //gerar laser
 if(keyDown("space") && tiros > 0 && tiros2 > 0){
     GerarL();
     tiros = tiros - 1;
     tiros2 = tiros2 - 1;
 ...
 if(nave.isTouching(muniçãoG) && tiros2 < 7){
     tiros2 += 3;
     muniçãoG.destroyEach();
    }
 else if(nave.isTouching(muniçãoG) && tiros2 > 6){
     muniçãoG.destroyEach();
     tiros2 = 9;
    }
